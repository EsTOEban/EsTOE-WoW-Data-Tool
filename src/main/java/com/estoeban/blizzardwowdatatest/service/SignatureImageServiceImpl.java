...
public class SignatureImageServiceImpl implements SignatureImageService {
     ...
     * to resolve.
     *
     * @param classId Id tied to in-game class
     ...
     *
     * @param portrait portrait of in-game character
     * @param baseImage image base
     * @return
     */
    ...
    protected BufferedImage addTextToImage(final BufferedImage baseImage, final CharacterItemsGuild characterItemsGuild, final ClassName className) throws IOException {
        ...
        try {
            ...
            try {
                ...
                localGraphicsEnvironment.registerFont(Font.createFont(Font.TRUETYPE_FONT, new File(
                        ClassLoader.getSystemClassLoader()
                                   .getResource("fonts/merriweather/Merriweather-Regular.ttf")
                                   .getFile()
                )));
                log.trace("Registering font Merriweather-Bold.ttf");
                localGraphicsEnvironment.registerFont(Font.createFont(Font.TRUETYPE_FONT, new File(
                        ClassLoader.getSystemClassLoader()
                                   .getResource("fonts/merriweather/Merriweather-Bold.ttf")
                                   .getFile()
                )));
                log.trace("Registering font Merriweather-BoldItalic.ttf");
                localGraphicsEnvironment.registerFont(Font.createFont(Font.TRUETYPE_FONT, new File(
                        ClassLoader.getSystemClassLoader()
                                   .getResource("fonts/merriweather/Merriweather-BoldItalic.ttf")
                                   .getFile()
                )));
                log.trace("Registering font Merriweather-Italic.ttf");
                localGraphicsEnvironment.registerFont(Font.createFont(Font.TRUETYPE_FONT, new File(
                        ClassLoader.getSystemClassLoader()
                                   .getResource("fonts/merriweather/Merriweather-Italic.ttf")
                                   .getFile()
                )));
            ...
            if (characterItemsGuild.getGuild() != null && characterItemsGuild.getGuild()
                                                                             .getName() != null && !characterItemsGuild.getGuild()
                                                                                                                       .getName()
                                                                                                                       .isEmpty()) {
                g2d.drawString(String.format("Level %s %s of <%s> on %s", characterItemsGuild.getLevel(), className.getName(), characterItemsGuild
                        .getGuild()
                        .getName(), characterItemsGuild.getRealm()), characterSummaryX, characterSummaryY);
            } else {
                g2d.drawString(String.format("Level %s %s on %s", characterItemsGuild.getLevel(), className.getName(), characterItemsGuild
                        .getRealm()), characterSummaryX, characterSummaryY);
            }
            ...
            g2d.drawString(String.format("Item Level: %s (%s)", characterItemsGuild.getItems()
                                                                                   .getAverageItemLevel(), characterItemsGuild
                    .getItems()
                    .getAverageItemLevelEquipped()), itemLevelDetailX, itemLevelDetailY);
